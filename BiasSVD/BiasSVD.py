...
class BiasSVD(object):
    ...
    def fit(self, ratings):
        ...
        self.P, self.Q, self.Biasuser, self.Biasitem = self.solve(ratings)

    def initMatrix(self):
        """
            @function: 初始化用户和物品隐语义矩阵以及用户无关偏置矩阵、物品无关偏置矩阵
            @return:初始化矩阵P，Q， BiasUser BiasItem
        """
        # User-LatentFactor
        P = dict(zip(
        ...
        # 初始化Biasuser、Biasitem的值，全部设为0
        Biasuser = dict(zip(self.users_ratings.index, np.zeros(len(self.users_ratings))))
        Biasitem = dict(zip(self.items_ratings.index, np.zeros(len(self.items_ratings))))

        return P, Q, Biasuser, Biasitem
    ...
    def solve(self, ratings):
        """
            ...
            @return: 模型参数P,Q
        """
        P, Q, biasUser, biasItem = self.initMatrix()
        for iteration in range(self.iterations):
            ...
            for user_id, item_id, rating in ratings.itertuples(index=False):
                p_i = P[user_id]
                q_j = Q[item_id]
                b_i = biasUser[user_id]
                b_j = biasItem[item_id]

                loss = np.float(rating - np.dot(p_i.T, q_j) - self.GLOBALMEAN - b_i - b_j)
                p_i += self.lr * (loss * q_j - self.penalty * p_i)
                q_j += self.lr * (loss * p_i - self.penalty * q_j)
                b_i += self.lr * (loss - self.penalty * b_i)
                b_j += self.lr * (loss - self.penalty * b_j)

                P[user_id] = p_i
                Q[item_id] = q_j
                biasUser[user_id] = b_i
                biasItem[item_id] = b_j
    ...
    def predict(self, user, item):
        ...
        if user not in self.P.keys() or item not in self.Q.keys():
            ...
            preference = np.dot(self.P[user].T, self.Q[item]) + self.GLOBALMEAN + self.Biasuser[user] + self.Biasitem[item]
        return preference
